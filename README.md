...
- **`internal/domain`**: Pure business logic, typed errors. Zero internal dependencies.
- **`internal/application`**: Ports (`UserRepository`, `PasswordHasher`) and Use Cases.
- **`internal/infra`**: SQLite + sqlc adapters, bcrypt password hasher.
- **`internal/api`**: Chi-based HTTP server + DTOs. Maps domain errors to HTTP status codes.
...
|--------|------|-------------|--------------|
| `POST` | `/register` | Register a new user | 201, 400, 409, 500 |
...
make verify
```
Runs architectural leak checks (depguard), vulnerability scanning, and workspace tests.

CI runs automatically on push/PR to `main` via `.github/workflows/ci.yml` with three jobs: **Backend** (lint, vuln check, tests with race detector, build), **Frontend** (lint, build), and **Docker** (image builds).
