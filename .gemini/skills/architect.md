...
- **Responsibility:** Business logic, core entities, and typed errors.
...
- **Guidelines:**
  ...
  - Define sentinel errors (`ErrEmailRequired`) and typed errors (`*UserAlreadyExistsError`) for classifiable failures.
...
- **Guidelines:**
  - Define interfaces (Ports) for persistence and external services here (e.g., `UserRepository`, `PasswordHasher`).
  - Cross-cutting concerns (password hashing, notifications) are modeled as ports with adapters in infra.
...
- **Guidelines:**
  ...
  - Maps domain errors to HTTP status codes using `errors.Is`/`errors.As` (e.g., 400 for validation, 409 for conflicts, 500 for internal).
